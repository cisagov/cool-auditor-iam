...
resource "aws_iam_user_group_membership" "financial_audit" {
  ...
  for_each = { for username, attributes in var.auditors : username => "" if contains(attributes["roles"], "financial_audit") }
...
resource "aws_iam_user_group_membership" "security_audit" {
  ...
  for_each = { for username, attributes in var.auditors : username => "" if contains(attributes["roles"], "security_audit") }
